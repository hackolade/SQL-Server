...
const objtypesToDescriptionCommentsInjectors = {
	SCHEMA: ({ schemas, objname, value }) =>
		schemas.map(collection =>
			collection.dbName === objname ? setObjectProperty(collection, 'bucketInfo.description', value) : collection,
		),
	TABLE: ({ schemas, objname, value, schema }) =>
		schemas.map(collection =>
			collection?.schemaName === schema && collection.collectionName === objname
				? setObjectProperty(collection, 'entityLevel.description', value)
				: collection,
		),
	VIEW: ({ schemas, objname, value, schema }) =>
		schemas.map(view =>
			view?.schemaName === schema && view.collectionName === objname
				? setObjectProperty(view, 'data.description', value)
				: view,
		),
	COLUMN: ({ schemas, objname, value, schema, entityName }) =>
		schemas.map(collection => {
			return !collection?.data &&
				...
				? setObjectProperty(
						collection,
						`collection.validation.jsonSchema.properties.${objname}.description`,
						value,
					)
				: collection;
		}),
...
const getJsonSchemasWithInjectedDescriptionComments = async ({ client, dbName, jsonSchemas, logger }) => {
	...
	const entities = jsonSchemas.map(({ collectionName: name, dbName, data }) => {
		...
		if (type === 'view') {
			schemasWithViews = schemasWithViews.map(schema =>
				schema.schema === dbName ? { schema: schema.schema, viewsCount: schema.viewsCount + 1 } : schema,
			);
		}
		if (type === 'table') {
			schemasWithTables = schemasWithTables.map(schema =>
				schema.schema === dbName ? { schema: schema.schema, tablesCount: schema.tablesCount + 1 } : schema,
			);
		}
		return {
			schema: dbName,
	...
	schemasWithViews = schemasWithViews
		.filter(schema => schema.viewsCount > 0)
		.map(({ schema }) => ({ schema, entity: { type: 'view' } }));
	schemasWithTables = schemasWithTables
		.filter(schema => schema.tablesCount > 0)
		.map(({ schema }) => ({ schema, entity: { type: 'table' } }));
	...
	descriptionComments.forEach(({ objtype, objname, value }) => {
		...
		jsonSchemasWithDescriptionComments = getSchemasWithInjectedDescriptionComments({
			schemas: jsonSchemasWithDescriptionComments,
			objname,
			value,
		});
	});
